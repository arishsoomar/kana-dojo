...
describe('makeQuestion', () => {
  it('asks an unlocked kana', () => {
    for (const rng of rngs) {
      const { kana } = makeQuestion(NEW_LEARNER, 'hiragana', rng);
  ...
  it('offers four choices that include the answer', () => {
    for (const rng of rngs) {
      const { kana, choices } = makeQuestion(NEW_LEARNER, 'hiragana', rng);
  ...
  it("only asks kana the learner has met, not ones a newly opened row hasn't taught", () => {
    ...
    const progress: Progress = {
      ...EMPTY_PROGRESS,
      kana: { あ: green, い: green, う: green, え: green, お: green, か: { box: 0, at: 0 } },
    };
    for (const rng of rngs) {
      const { kana } = makeQuestion(progress, 'hiragana', rng);
      expect(['あ', 'い', 'う', 'え', 'お', 'か']).toContain(kana.char);
    }
  });

  it('uses the requested script', () => {
    const { kana, choices } = makeQuestion(NEW_LEARNER, 'katakana', () => 0.5);
...
describe('makeDrillQuestion', () => {
  const KANA_ROW = { あ: { box: 3, at: 0 }, い: { box: 3, at: 0 }, う: { box: 3, at: 0 }, え: { box: 3, at: 0 } };
  const progress: Progress = {
    ...EMPTY_PROGRESS,
    kana: KANA_ROW,
  ...
  it('asks the drilled kana about half the time', () => {
    ...
    expect(makeDrillQuestion(progress, focus, () => 0.2).kana).toBe(focus);
  });

  it('otherwise asks a kana it gets mixed up with', () => {
    if (!focus) throw new Error('あ should be unlocked');
    expect(makeDrillQuestion(progress, focus, () => 0.7).kana.char).toBe('お');
  });

  it('offers four choices that include the answer', () => {
    if (!focus) throw new Error('あ should be unlocked');
    const { kana, choices } = makeDrillQuestion(progress, focus, () => 0.7);
    expect(choices).toHaveLength(4);
    expect(choices).toContain(kana);
  });
});
...
describe('never the same kana twice in a row', () => {
  ...
  it('makeQuestion avoids the kana just asked', () => {
    for (const rng of rngs) {
      expect(makeQuestion(NEW_LEARNER, 'hiragana', rng, 'あ').kana.char).not.toBe('あ');
    }
  });
  ...
  it('makeDrillQuestion asks a partner instead of repeating the drilled kana', () => {
    ...
    if (!focus) throw new Error('あ should be unlocked');
    for (const rng of rngs) {
      expect(makeDrillQuestion(progress, focus, rng, 'あ').kana.char).toBe('お');
      expect(makeDrillQuestion(progress, focus, rng, 'お').kana.char).toBe('あ');
    }
  });
  ...
  it('still drills a kana with no partners, since it is the only one', () => {
    ...
    expect(makeDrillQuestion(NEW_LEARNER, focus, () => 0.9, 'い').kana.char).toBe('い');
  });
});
